...
from model_IJPEA import IJEPA_DualImages_Full
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrained_model_path', type=str, default='ckpts/IJEPA/vit-t-mae_8layers_patch16.pt')
    ...
    args.output_model_path = f'ckpts/IJEPA/vit-t-classifier-from_scratch_8layers_{args.dataset}_moe.pt'
    ...
    writer = SummaryWriter(os.path.join('logs', args.dataset, 'IJPEA-cls'))
    # IJEPA_model: IJEPA_DualImages_Full = IJEPA_DualImages_Full(img_size=224,patch_size=16,in_chans=3,embed_dim=192,enc_depth=8,pred_depth=4,num_heads=3,M=4,mode='test').to('cuda') 
    ...
    IJEPA_model: IJEPA_DualImages_Full = IJEPA_DualImages_Full(img_size=224,patch_size=16,in_chans=3,embed_dim=192,enc_depth=8,pred_depth=4,num_heads=3,M=4,mode='test').to('cuda') 
    IJEPA_model.load_state_dict(torch.load('/root/workspace/SVM/ckpts/IJEPA/vit-t-mae_8layers_patch16.pt'))
    model = IJPEA_Classifer(encoder=IJEPA_model, num_classes=args.num_classes).to('cuda')
